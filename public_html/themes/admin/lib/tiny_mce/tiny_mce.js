(function(e) {
    ...
    var c = {majorVersion: "3", minorVersion: "5.5", releaseDate: "2012-07-19", _init: function() {
...
tinymce.util.Quirks = function(d) {
    ...
    function g() {
        var I, H, X, G, S, V, T, W, J, U, Q, P, R = document, N = d.getDoc();
        if (!s.object_resizing || s.webkit_fake_resize === false) {
            return
        }
        c("enableObjectResizing", false);
        U = {n: [0.5, 0, 0, -1], e: [1, 0.5, 1, 0], s: [0.5, 1, 0, 1], w: [0, 0.5, -1, 0], nw: [0, 0, -1, -1], ne: [1, 0, 1, -1], se: [1, 1, 1, 1], sw: [0, 1, -1, 1]};
        function L(ac) {
            var aa, Z, ab;
            aa = ac.screenX - V;
            Z = ac.screenY - T;
            ab = Math.max((W + aa) / W, (J + Z) / J);
            if (m.modifierPressed(ac)) {
                Q = Math.round(W * ab);
                P = Math.round(J * ab)
            } else {
                Q = aa * S[2] + W;
            ...
            Q = Q < 5 ? 5 : Q;
            P = P < 5 ? 5 : P;
            p.setStyles(G, {width: Q, height: P});
            if (S[2] < 0 && G.clientWidth <= Q) {
                p.setStyle(G, "left", I + aa)
            }
            if (S[3] < 0 && G.clientHeight <= P) {
                p.setStyle(G, "top", H + Z)
            }
        }
        function Y() {
            function Z(aa, ab) {
                if (ab) {
                    if (X.style[aa] || !d.schema.isValid(X.nodeName.toLowerCase(), aa)) {
                        p.setStyle(X, aa, ab)
                    } else {
                        p.setAttrib(X, aa, ab)
                    }
            ...
            Z("width", Q);
            Z("height", P);
            p.unbind(N, "mousemove", L);
            p.unbind(N, "mouseup", Y);
            if (R != N) {
                p.unbind(R, "mousemove", L);
                p.unbind(R, "mouseup", Y)
            }
            p.remove(G);
            K(X)
        }
        function K(ac) {
            ...
            tinymce.each(U, function(af, ad) {
                var ae;
                ae = p.get("mceResizeHandle" + ad);
                if (!ae) {
                    ae = p.add(N.documentElement, "div", {id: "mceResizeHandle" + ad, "class": "mceResizeHandle", style: "cursor:" + ad + "-resize; margin:0; padding:0"});
                    p.bind(ae, "mousedown", function(ag) {
                        ag.preventDefault();
                        Y();
                        V = ag.screenX;
                        T = ag.screenY;
                        ...
                        N.documentElement.appendChild(G);
                        p.bind(N, "mousemove", L);
                        p.bind(N, "mouseup", Y);
                        if (R != N) {
                            p.bind(R, "mousemove", L);
                            p.bind(R, "mouseup", Y)
                        }
                    })
                } else {
                ...
                p.setStyles(ae, {left: (ab * af[0] + I) - (ae.offsetWidth / 2), top: (Z * af[1] + H) - (ae.offsetHeight / 2)})
            });
            if (!tinymce.isOpera && X.nodeName == "IMG") {
                X.setAttribute("data-mce-selected", "1")
            }
        }
        function M() {
            if (X) {
                X.removeAttribute("data-mce-selected")
            }
            for (var Z in U) {
                p.hide("mceResizeHandle" + Z)
            }
        }
        ...
        function O() {
            var Z = p.getParent(E.getNode(), "table,img");
            tinymce.each(p.select("img[data-mce-selected]"), function(aa) {
                aa.removeAttribute("data-mce-selected")
            });
            if (Z) {
                K(Z)
            } else {
        ...
        d.onNodeChange.add(O);
        p.bind(N, "selectionchange", O);
        d.serializer.addAttributeFilter("data-mce-selected", function(Z, aa) {
            var ab = Z.length;
            while (ab--) {
                Z[ab].attr(aa, null)
            }
